...
class Transformer:
    ...
    def unsupervised_parameters(self):
            return [
    ...
    def encoder_layer(self, input):
        ...
        rc_output = residual_connection(input, final_projection)
        ...
        ff_residual_output = residual_connection(ln_output, ff_output)
...
if __name__ == "__main__":
    ...
    # One unsupervised sample
    unsupervised_sample = (
        unsupervised_training_dataset[0]
        .unsqueeze(0)
    )
    ...
    ) = transformer.unsupervised_reconstruction(
        unsupervised_sample
    )

    # One supervised sample
    supervised_sample, supervised_label = (
        supervised_training_dataset[0]
    )

    supervised_sample = (
        supervised_sample.unsqueeze(0)
    )

    supervised_label = (
        supervised_label.unsqueeze(0)
    )
    ...
    ) = transformer.supervised_fine_tuning(
        supervised_sample,
        supervised_label
    )

    print(
        "Unsupervised sample shape:",
        unsupervised_sample.shape
    )
    ...
    print(
        "Supervised sample shape:",
        supervised_sample.shape
    )

    print(
        "Supervised label shape:",
        supervised_label.shape
    )

    print(
        "True label index:",
        supervised_label.item()
    )

    print(
        "True label name:",
        supervised_training_dataset.index_to_label[
            supervised_label.item()
        ]
    )
